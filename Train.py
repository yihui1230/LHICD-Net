...
def val(args, val_loader, model, epoch):
    ...
    for iter, batched_inputs in enumerate(val_loader):
        ...
        img, target,e_target = batched_inputs
        pre_img = img[:, 0:3]
        post_img = img[:, 3:6]
        ...
        outputs = model(pre_img_var,post_img_var)
        outputs=_resize(outputs)
        loss = BCEDiceLoss(outputs, target_var)
        pred = torch.where(outputs > 0.5, torch.ones_like(outputs), torch.zeros_like(outputs)).long()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_steps', type=int, default=59300*3, help='Max. number of iterations')
